...
<div class="row">
    <div class="col-md-12" id="video_link_id">
        <div class="formrow">
            <label for="">{{__('Video File')}}</label>
            @if(!empty($user->bio_video))
                <video width="320" height="240" controls>
                    <source src="{{ url('user_videos/'.$user->bio_video) }}" type="video/mp4">
                    Your browser does not support the video tag.
                </video>
            @endif
            <input type="file" name="bio_video" id="bio_video" class="form-control">
            {!! APFrmErrHelp::showErrors($errors, 'bio_video') !!}
        </div>
    </div>
</div>
<hr>
